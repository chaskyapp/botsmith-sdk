...
- **S6 — Developer keys instead of the platform secret.** *Not a papercut like
  S2–S5: it decides whether third parties can administer their own bots at all.*

  The admin surface authenticates with `X-Secret`, the **global** API secret
  that guards every non-public route. That makes a leak unattributable, makes
  rotation all-or-nothing, and means no external developer can ever be given
  access — which is why the admin client had to be split into a package only
  Chasky can use (§6).

  A `sk_`-prefixed key, generated by a developer from a panel, hashed at rest,
  revealed once, **scoped to the bots its owner owns**, and authenticated
  separately from `ValidateApiSecret`, removes all three. Full proposal in
